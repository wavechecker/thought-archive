 ...
 *   /api/x402/visit-prep          — Paid structured appointment-prep checklists (Base/EVM)
 *   /api/x402/solana/visit-prep   — Paid structured appointment-prep checklists (Solana Devnet)
 *
 * SOLANA DEVNET RAIL:
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // ── /api/x402/visit-prep ─────────────────────────────────────────────────
    if (url.pathname === "/api/x402/visit-prep") {
      const slug = url.searchParams.get("slug");
    ...
    if (url.pathname === "/api/x402/solana/visit-prep") {
      const solanaPayTo = env.X402_SOLANA_RECEIVING_ADDRESS?.trim();
      if (!solanaPayTo) {
        return jsonError(503, "x402_solana_not_configured");
      }
