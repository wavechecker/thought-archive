  GNU nano 8.5                                                                                                              scripts/backfill-dates.mjs
...
for (const f of files) {
  ...
  let { publishDate, updatedDate } = fm.data;

  if (!publishDate) {
    publishDate = gitDateISO(f, "A") || fileMtimeISO(f) || new Date().toISOString();
    fm.data.publishDate = publishDate;
  }

  if (!updatedDate) {
    updatedDate = gitDateISO(f, "M") || publishDate;
    fm.data.updatedDate = updatedDate;
  }
